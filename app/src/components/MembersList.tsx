  ...
  if (members.length === 0) {
    return (
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-card">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-400">Members</h3>
    ...
    <section className="rounded-2xl border border-border bg-surface p-5 shadow-card">
      ...
      <ul className="divide-y divide-border">
        {members.map((m) => {
          ...
          return (
            <li key={m.slotIndex} className="flex items-center gap-3 py-2.5 text-sm">
              <span className={`h-2.5 w-2.5 rounded-full ${slotDot[m.status]}`} aria-hidden />
              <span className="w-10 font-mono text-xs text-slate-500">
                #{m.slotIndex.toString().padStart(2, "0")}
              </span>
              <span className="flex-1 truncate">
                <span className="font-medium">{m.name}</span>
                <span className="ml-2 text-xs text-slate-500">L{m.reputationLevel}</span>
                {isClaimantThisCycle ? (
