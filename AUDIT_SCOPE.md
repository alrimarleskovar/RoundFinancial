...
| **Total in scope**              | **15,793 LoC** | **91 typed errors** (`RoundfiError`; codes are positional — `6000 + index` — so variants are only ever appended), **879 tests across 70 spec files** (security-specific bankrun + 95 app-encoder structural + bankrun round-trips + canary-control negative + audit-regression unit/proptest + 12 litesvm lifecycle specs + 59 lance unit tests) + **6 cargo-fuzz targets** on `roundfi-math` (~9.85B inputs cumulative — 503M historical CI smoke + 600M afternoon re-validation 2026-05-24 + 8.75B overnight sweep 2026-05-24 (6 targets × 1h each), 0 crashes across entire history, coverage stable (saturation reached) in the input space covered by the current corpus), Triple Shield economic invariants + `bankrun_compat` shim (ADR 0007) enabling cooldown-bound + time-warp specs to run in seconds rather than 7-day real waits |
