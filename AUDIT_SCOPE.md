...
## In scope — 3 Anchor programs · ~15,793 lines of Rust

| Program                         | LoC (`*.rs`)   | Surface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `programs/roundfi-core`         | **10,444**     | **40 instructions · 6 account types** (`ProtocolConfig`, `Pool`, `Member`, `Listing`, `DrawResult`, `Bid`). Pool lifecycle (create / join / contribute / claim_payout / crank_payout / settle_default / skip_defaulted_payout / close), escrow + solidarity + yield vault PDAs, escape valve secondary market (commit-reveal list/buy), treasury timelock + lock, harvest waterfall, Triple Shield invariants, **and the contemplation subsystem — `finalize_draw` (sorteio → `DrawResult` permutation), `place_embedded_bid`, `place_bid_commit` + `place_bid_reveal` (sealed free bid). See [`docs/security/lance-contemplation.md`](./docs/security/lance-contemplation.md); its §5.5 questions are open and are exactly where audit hours pay off.**                                                                                                                                  |
| `programs/roundfi-reputation`   | **4,124**      | SAS-compatible attestations, level promotion (1→2→3→4), CPI surface from roundfi-core, identity scaffold (Civic gateway-token validator — provider TBD post-mainnet, see [§4.4 of architecture.md](./docs/architecture.md#44-identity-layer-added-v02--2026-04-22--provider-transition-v04--2026-05)). **`BehavioralPayload.parcels_paid: u8`** lets a K-installment lance settle with one attestation rather than K — new surface for double-count / farming analysis.                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `programs/roundfi-yield-kamino` | **1,225**      | Real Kamino Lend CPI — `deposit_reserve_liquidity` (deposit path) + `redeem_reserve_collateral` (harvest path, **redeem-all + redeposit-principal** round-trip; see module comment) — production target                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Total in scope**              | **15,793 LoC** | **91 typed errors** (`RoundfiError`; codes are positional — `6000 + index` — so variants are only ever appended), **886 tests across 90 spec files** (70 in `tests/` + 20 in `services/*/test/`) (security-specific bankrun + 95 app-encoder structural + bankrun round-trips + canary-control negative + audit-regression unit/proptest + 12 litesvm lifecycle specs + 59 lance unit tests) + **6 cargo-fuzz targets** on `roundfi-math` (~9.85B inputs cumulative — 503M historical CI smoke + 600M afternoon re-validation 2026-05-24 + 8.75B overnight sweep 2026-05-24 (6 targets × 1h each), 0 crashes across entire history, coverage stable (saturation reached) in the input space covered by the current corpus), Triple Shield economic invariants + `bankrun_compat` shim (ADR 0007) enabling cooldown-bound + time-warp specs to run in seconds rather than 7-day real waits |

---

### Newest surface — start here (added Jul 2026)

The **contemplation subsystem** ([ADR 0012](./docs/adr/0012-contemplation-lance-and-prepayment.md)) is the least-weathered code in scope and has **not** been through an internal red-team pass of its own. Suggested focus, in order:

1. **`place_bid_reveal` ordering.** Eligibility is adjudicated (`depth > pool.current_bid_depth`) **before** any token transfer — that ordering is what makes losing bids revert cleanly and is why the design has no bid vault and no refund path. Moving the check after the transfers silently reintroduces a refund obligation with no code to honor it.
2. **The bid-depth formula.** `depth = contributions_paid − current_cycle − 1`. The `−1` distinguishes _ahead_ from merely _current_; dropping it lets a current member bid with zero prepayment.
3. **The swap.** A winning bid transposes two entries of `DrawResult.order`. The "everyone is contemplated exactly once" property rests on that staying a permutation — worth checking against every path that writes `order`.
4. **Window disjointness.** Commit requires `clock < next_cycle_at`; reveal requires `next_cycle_at ≤ clock < next_cycle_at + GRACE_PERIOD_SECS`. This pair _is_ the anti-snipe property; any overlap breaks it.
5. **`finalize_draw` is not pause-gated** — it takes no `config` account at all. It moves no funds and every paying instruction downstream _is_ gated, but this is a structural carve-out rather than a deliberate one (unlike `settle_default` / SEV-018) and we would rather it be reviewed than discovered.
6. **`parcels_paid > 1`** in `BehavioralPayload` — one attestation covering K installments. Worth confirming it cannot be replayed or double-counted by the reputation engine or the indexer.

Open design questions the team has **not** resolved are enumerated in [`docs/security/lance-contemplation.md`](./docs/security/lance-contemplation.md) §5.5.
