...
| `tripleVeteranDefault` | **24**    | **Veterano** | **$10,000**   | Members 1/2/3 contemplated at cycles 2/3/4, each defaults at the cycle right after receiving their upfront                             | **Canonical whitepaper stress test**: gross liability $30,000; loss bounded by retained capital (escrow $19,500 + stake slash $3,000 + Seed Draw cushion $9,152 — redistributed buffers, not net surplus) **= loss-bounded by construction; surplus is yield-backed** (verified: healthy 0% yield = $0 zero-sum, 6.5% APY = +$2,756, triple-default 6.5% ≈ +$28). Do **not** read these buffers as a "+$4,152 solvent at 0% yield" net — that figure is refutable in the public Stress Lab (ECO-005). |
| `eliteTripleDefault`   | **24**    | **Elite**    | **$10,000**   | Same 24 / $10k / 3-post-contemplation-default shape as `tripleVeteranDefault`, but at the **Lv4 Elite** tier (3% stake = 33× leverage) | **ECO-V52-1**: models the thin-stake tail the audit flagged. The 3% stake recovers less collateral per default, so end-of-life `netSolvency` is **strictly worse** than Veterano (auditor measured ≈ −$1.8k vs ≈ +$28 at 6.5% APY). `netSolvency` is **display-only** (ECO-001) — on-chain loss stays bounded by the D/C + Seed-Draw invariants at every tier; the point is to stop extending "solvent by construction" to the 3% tier without showing this.                                          |
...
The canonical 6 above cover N=12 and N=24 (the last two isolate the Veterano-10% vs Elite-3% tier tail). The 11 extended presets below close the gap across **pool-size**, **tier-mix**, **default-position**, and **yield-extreme** dimensions. Each preset runs the existing 34-invariant parametric loops + a dedicated smoke test under the `L1 stress-lab sanity — extended presets (Issue #228)` describe block.
...
- **Tier:** Iniciante (50% stake), Comprovado (25%), Veterano (10%), Elite (3%)
