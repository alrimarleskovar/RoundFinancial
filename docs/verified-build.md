...
> **⚠ Status note (post-#487, 2026-06-14).** The 4 devnet programs were upgraded **in-place** by the Agave 3.x / anchor 1.0 / mpl-core 0.12 migration (#487) using a non-reproducible `anchor build`, so verify-build attestations recorded **before** #487 are now **stale** — `solana-verify get-program-hash <pid>` (deployed) no longer matches the attested hash. **Refresh status** (probed 2026-06-14, `solana-verify 0.4.15`): the **Solana 3.0.0 verifiable-build image exists and pulls fine**, but its pinned platform-tools **Cargo 1.84.0 cannot build `cpufeatures 0.3.0`** (needs `edition2024` / Cargo ≥1.85), which is pulled via `blake3 1.8.5 ← solana-blake3-hasher 3.1.0 ← solana-program 3.0.0`. So the refresh is blocked on a **transient toolchain gap**, not a missing image. Unblock by either **(a)** waiting for an updated verifiable-build image with Cargo ≥1.85 (zero repo change — preferred for a devnet badge), or **(b)** pinning `blake3` down to a pre-`cpufeatures-0.3` release (`cargo update -p blake3 --precise 1.5.5`, verified to drop 0.3.0) — but that's a project-wide build-input change that also forces a `verify-redeploy`, so weigh it against just waiting.
