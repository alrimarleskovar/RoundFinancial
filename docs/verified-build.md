...
> **✅ Status note (resolved 2026-06-22).** The 4 devnet programs were upgraded **in-place** by the Agave 3.x / anchor 1.0 / mpl-core 0.12 migration (#487) using a non-reproducible `anchor build`, so verify-build attestations recorded **before** #487 went **stale** — `solana-verify get-program-hash <pid>` (deployed) no longer matched the attested hash. The 2026-06-14 refresh (#492) was then **blocked on a toolchain gap**: `solana-verify` resolved the build image from the `solana-program 3.0.0` pin in `Cargo.lock`, landing on `solana-verifiable-build:3.0.0` whose platform-tools ship **Cargo 1.84.0** — too old for the `edition2024` crates the anchor-1.0 / solana-3.0 tree pulls (`cpufeatures 0.3.0`, `base64ct 1.8.3`, …). Pinning those crates down was the wrong fix (whack-a-mole, changes build inputs project-wide). **Resolution:** the `solana-verifiable-build:3.1.14` image (published 2026-05-04) ships **platform-tools v1.52 / rustc 1.89** — the SAME toolchain the deploy `anchor build` uses — and builds the `edition2024` tree cleanly. `Cargo.toml` now pins it explicitly via `[workspace.metadata.cli] solana = "3.1.14"` (top of `solana-verify`'s image-selection precedence, ahead of the `Cargo.lock` fallback), so the verify build reproduces the exact on-chain bytecode with **no dep pins**. One wrinkle: the `:3.1.14` image (published 2026-05-04) is newer than `solana-verify 0.4.15` (March), so the CLI doesn't carry that tag in its built-in known-image list and bails with `No compatible Docker image found for Solana version 3.1.14`. The fix is the `--base-image` flag the error itself recommends — `scripts/devnet/verify-build.sh` now derives the tag from the `metadata.cli` pin and passes it automatically, so `pnpm devnet:verify-build` works on `solana-verify ≥ 0.4.14` (the release that added `metadata.cli` parsing) regardless of how new the pinned image is. **Validated 2026-06-22:** `roundfi_core` built clean inside the `:3.1.14` image (`rustc 1.89.0-sbpf-solana-v1.52`), hash `ee8ac35…276d`. Operationally: re-run `pnpm devnet:verify-build` (Step 1 below) on a host with Docker. Keep the `solana = "x.y.z"` pin in lockstep with the deploy toolchain (`solana --version`) on every future redeploy.
...
**`solana-verify build` fails with `No compatible Docker image found for Solana version 3.1.14`**
→ Your `solana-verify` predates the pinned image's publish date, so it doesn't carry that tag in its known-image list yet. `scripts/devnet/verify-build.sh` already passes `--base-image solanafoundation/solana-verifiable-build:<pin>` (derived from `[workspace.metadata.cli] solana`), so `pnpm devnet:verify-build` handles it. If you call `solana-verify build` directly, add the same flag: `--base-image solanafoundation/solana-verifiable-build:3.1.14`. (Or `cargo install solana-verify --locked` to a release dated after the image, which makes the bare `metadata.cli` pin resolve on its own.)

**`solana-verify build` fails on `cpufeatures` / `base64ct` / `edition2024` (needs Cargo ≥1.85)**
→ The build image is too old (e.g. `:3.0.0` ships Cargo 1.84). The `metadata.cli` pin / `--base-image` isn't selecting the 3.1.x image. Confirm `Cargo.toml` has `[workspace.metadata.cli] solana = "3.1.14"` and that the build log's first lines name `solana-verifiable-build:3.1.14` (not `:3.0.0`). Do **not** pin the edition2024 crates down — that's whack-a-mole that changes build inputs project-wide; fix the image instead (#492 lineage).
