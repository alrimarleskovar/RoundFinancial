...
> **⚠ Status note (post-#487, 2026-06-14).** The 4 devnet programs were upgraded **in-place** by the Agave 3.x / anchor 1.0 / mpl-core 0.12 migration (#487) using a non-reproducible `anchor build`, so verify-build attestations recorded **before** #487 are now **stale** — `solana-verify get-program-hash <pid>` (deployed) no longer matches the attested hash. **Refresh status** (probed 2026-06-14, `solana-verify 0.4.15`): the **Solana 3.0.0 verifiable-build image exists and pulls fine**, but its pinned platform-tools **Cargo 1.84.0 cannot build `cpufeatures 0.3.0`** (needs `edition2024` / Cargo ≥1.85), which is pulled via `blake3 1.8.5 ← solana-blake3-hasher 3.1.0 ← solana-program 3.0.0`. So the refresh is blocked on a **toolchain gap, not a missing image — and it's systemic, not a single crate.** Pinning `blake3` down clears `cpufeatures 0.3.0`, but the build then fails on `base64ct 1.8.3` (also `edition2024`), and more would follow: the anchor-1.0 / Solana-3.0 dep tree broadly adopted `edition2024` (Cargo ≥1.85), which the image's pinned **Cargo 1.84.0** predates. Pinning each edition2024 crate down is whack-a-mole and changes build inputs project-wide. **The only clean path is to wait for an updated `solana-verifiable-build` image with Cargo ≥1.85**, then re-probe (`pnpm devnet:verify-build roundfi_core`). Not worth chasing for a devnet badge; the deployed bytecode + source are public regardless.
