...
(L2 = 2, L3 = 3, L4 = 8). Under the Pass-3 taxonomy `cycles_completed` only
increments on `SCHEMA_POOL_COMPLETE` — emitted once per pool the member paid
through to the **end** — and carries a 30-day per-subject cooldown
(`MIN_POOL_COMPLETE_COOLDOWN_SECS`). So each unit is a full ROSCA round, not
a single payment: L2 needs 2 completed pools (ECO-V52, raised from 1), L3
needs 3 (≈ a year-plus of real history), L4 needs 8 (≈ four years) — which
destroys the "L3 in hours" economics by an even wider margin than the
original 6-day-cycle design did. This layer has no configuration and no off
switch: it protects devnet, Canary, and mainnet identically. Residual
farming vectors checked against it: **rejoin** (cycles_completed is
additive-only, never resets → no bypass), **micro-cycles** (each PoolComplete
still burns the 30-day cooldown regardless of pool size → gate holds),
**self-referral** (referral is off-chain, never touches on-chain score →
outside this vector).
...
| Reputation farming (low-installment attestations) | mitigated — identity gate shipped (SEV-047) + the Pass-3 cycles floor; residual = oracle-side counting. Two-layer gate at `promote_level`: (1) cycles gate (always-on) requires `cycles_completed` ≥ N completed pools (L2=2, L3=3, L4=8), each emitted only at end-of-pool with a 30-day per-subject cooldown ⇒ months-to-years of real history, blocking the PAYMENT-spam path; (2) identity gate (config-gated, default-OFF) caps unverified subjects below `required_min_level` | Installment floor + B2B oracle USD-weighted reads; mainnet sets identity-gate `required_min_level` (2 or 3) as defense-in-depth — wiring shipped, value is an operational decision |
...
_Last updated: 2026-05-26 · pre-mainnet draft._
