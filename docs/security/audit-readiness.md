...
> - [`./internal-audit-findings.md`](./internal-audit-findings.md) — **internal pre-audit tracker (canonical counts in its [Summary table](./internal-audit-findings.md#summary); 51 findings / 47 closed / 1 OPEN (SEV-050, High — mainnet blocker) / Critical/High 15 of 16 as of 2026-05-26)** — read this first to see what the team's own red-team already surfaced
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Internal pre-audit (external-auditor methodology)** | **51 findings catalogued, 47 🟢 closed, 1 🟡 OPEN (SEV-050, High — mainnet blocker), 0 🟠 upstream-blocked, 3 🔵 design-intentional** (live counts canonical in the tracker [Summary table](./internal-audit-findings.md#summary)) | [`internal-audit-findings.md`](./internal-audit-findings.md) — public tracker, SEV-001..SEV-050 + SEV-034b                                                                                |
| **Critical / High fixes**                             | **15 of 16 🟢 closed** (6 Critical + 10 High; the 1 open is SEV-050 High — a deferred defaulted-pool `close_pool` liveness blocker) with negative regression tests per "test-before-merge" gate                                    | PRs [#326..#413](https://github.com/alrimarleskovar/RoundFinancial/pulls?q=is%3Apr+is%3Amerged+SEV) + 2026-05-24 external-audit pass (SEV-047/048) + litesvm L1↔L2 parity slice (SEV-049) |
...
> 🟡 The one open finding is **SEV-050 (High)** — a defaulted pool cannot `close_pool` (`escrow_balance` never drains to 0), stranding funds; fix deferred, a mainnet blocker until closed. SEV-049 (High, pre-contemplation-default liveness) is closed via the new permissionless `skip_defaulted_payout` instruction.
...
See [`internal-audit-findings.md`](./internal-audit-findings.md) for the per-finding status. **The auditor's W4 evaluation cleared every fund-loss-shaped finding it reached as 🟢 closed; the one fund-loss-shaped finding still open is SEV-050 (High) — a defaulted pool cannot `close_pool` because `escrow_balance` never drains to 0, stranding funds; fix deferred, a mainnet blocker until closed.**
...
The internal pre-audit (W1..W5 + 1 integration-testing wave + 9 follow-up waves + 1 external-audit pass + litesvm L1↔L2 parity slice, 51 findings, 47 closed, 1 🟡 OPEN — SEV-050, High — a mainnet blocker) ran _before_ commissioning the formal external engagement. Recommended formal scope:
...
- Internal pre-audit tracker — [`internal-audit-findings.md`](./internal-audit-findings.md) (51 SEVs, 47 closed, 1 🟡 OPEN — SEV-050, High — mainnet blocker, 0 upstream-blocked, 3 design-intentional; canonical [Summary table](./internal-audit-findings.md#summary))
