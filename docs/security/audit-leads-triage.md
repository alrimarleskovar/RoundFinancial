...
| 002  | `create_pool` param validation                                      | High         | 🟡 PARTIAL    | Div-by-zero structurally impossible, overflow guarded, cycle-0 closed (SEV-031/038). On-chain negative tests **added** (#601); `pool_is_viable` Rust fuzz still open.                                                   |
...
| 005  | `escape_valve_buy` race                                             | (flagged)    | 🟡 PARTIAL    | Buy/cancel/rebuy loop doesn't exist; race resolved. Commit-reveal + cancel **now tested** (#602); two-buyer race needs the mpl-core lane; 2 dead enum states + Active-listing liveness (D3) remain.                     |
| 006  | Treasury lock state machine                                         | High         | 🟡 PARTIAL    | Rotations deadlock-free by construction; treasury rotation state-machine **now tested** (#600). `pause` freeze = D2; authority/fee trios follow the same shape.                                                         |
...
| 008  | Reputation farming economics                                        | High         | 🟢/🟡         | `crank_payout` adds no farmable surface — score-neutrality now **pinned in CI** (#603). Default-config farm uneconomical. Doc numbers fixed; R1 structural cap = D4.                                                    |
...
**Tracked dependencies for D2 (pin — Yvyna, 2026-07-05).** The whole thing is gated on **[#266](https://github.com/alrimarleskovar/RoundFinancial/issues/266)** (Squads 3-of-5 deploy + authority handoff — a mainnet-canary hard gate, `docs/operations/mainnet-canary-plan.md:48,75`). Today the pause authority (`config.authority`) is the deployer key, not Squads; the rotation mechanism (`propose/commit_new_authority`) is ready and was rehearsed on a parallel devnet deploy (2026-05-16), but the canonical handoff has not run. **Two parts must not be lost:**

1. **Break-glass sub-decision (still open).** Whether the **pause authority** should sit on a separate break-glass sub-multisig (R3 of `docs/operations/multisig-recovery.md:222`, `docs/status.md:105`) is an open design decision _inside_ #266 — not settled by the migration itself.
2. **Account-migration pre-requisite.** The canonical devnet `ProtocolConfig` is pre-#323 (317 vs 381 bytes), so `migrate_protocol_config` (core) must be **run** to realloc it **before** the Squads rotation can execute (`docs/status.md:104`). This is purely _operational_ — the instruction already carries its discriminator guard (`migrate_protocol_config.rs:114`). It is **distinct from the LEAD-011 finding**, which is the _sibling_ `migrate_reputation_config` (reputation, #408) missing that same guard: same design pattern, but a different program and a **separate** fix. If the mainnet setup also migrates the `ReputationConfig` (it grew for Human Passport, `architecture.md:324`), the LEAD-011 fix should land _before_ that migration runs so the guard protects it too.
...
- **Phase E — test batteries (DONE):** LEAD-006 treasury rotation (#600), LEAD-002 create_pool guards (#601), LEAD-005 escape-valve commit-reveal + cancel (#602), LEAD-008 PAYOUT_CLAIMED score-neutrality (#603) — all green in the bankrun lane. Not included (future): the `pool_is_viable` Rust fuzz target, the two-buyer `escape_valve_buy` race (needs the mpl-core lane), and the authority/fee rotation trios (same shape as #600).
