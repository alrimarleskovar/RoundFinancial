...
| **Upgrade authority controlled**                                           | Solana program-loader `ProgramData.upgrade_authority` set to deployer pubkey `64XM177Vm6zirzQnjU1juQ9TLqDsZVsCcZzfgEgVCffm` on all 4 deployed devnet programs. **Mainnet plan:** rotate to 3-of-5 Squads multisig PDA before mainnet smoke. Procedure shipped at [`docs/operations/squads-mainnet-ceremony-checklist.md`](../operations/squads-mainnet-ceremony-checklist.md); rehearsed on devnet 2026-05-16 ([rehearsal log](../operations/rehearsal-logs/2026-05-16-squads-rotation-rehearsal.md)). Post-rotation, the `pause` flow is documented across [`docs/operations/emergency-response.md §"Who can pause"`](../operations/emergency-response.md) (single-keypair seconds-to-minutes vs Squads 3-of-5 5-30 minutes, time-to-pause budget). | Runtime verification: `solana program show <pid> --url devnet` → "Upgrade Authority" field. Same pubkey across all 4 program IDs. Mainnet IDs come from `config/program-ids.mainnet-beta.json` (artifact uploaded by `.github/workflows/mainnet-deploy.yml` — SEV-046 CD pipeline).                                                                                                                                                                                                                                                   |
...
| **App-side encoder bytes match on-chain `<Accounts>` shape exactly**       | Front-end IDL-free encoders under `app/src/lib/*.ts` (6 builders) are structurally pinned: discriminator = `sha256("global:<ix>")[:8]`, account count = `<Accounts>` struct length, signer at index 0, PDA derivations via `@roundfi/sdk/pda`. Same encoders consumed by the indexer + tests.                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **58 structural tests** at `tests/app_encoders.spec.ts` — discriminator + account count + signer position + PDA derivation per encoder. **8 bankrun round-trips** at `tests/app_encoders_bankrun.spec.ts` — app-built instructions sent through `BanksClient.processTransaction`, full state delta asserted on contribute / claim_payout / release_escrow / escape_valve_list / settle_default / deposit_idle_to_yield. Plus **3 negative-path tests** prove on-chain guards correctly reject the same encoder against invalid state. |
| **Math layer fuzz-tested for coverage-guided edge cases**                  | `crates/math/` (extracted from `programs/roundfi-core/src/math/` per [ADR 0004](../adr/0004-extract-roundfi-math-crate.md)) compiles host-side. `cargo-fuzz` lane covers all 6 modules: cascade, waterfall, dc_invariant, escrow_vesting, bps, seed_draw. **proptest** (uniform-sample) runs on every `cargo test` (~1500 cases); **cargo-fuzz** (coverage-guided mutation) runs 60s/target per PR + 30min/target weekly. Sister projects (`subtle`, `ring`, `solana-program`'s borsh decode) ship both for the same reason.                                                                                                                                                                                                                         | **6 fuzz targets** at `crates/math/fuzz/fuzz_targets/` — see [`.github/workflows/fuzz.yml`](../../.github/workflows/fuzz.yml) (60s PR smoke) + [`.github/workflows/fuzz-scheduled.yml`](../../.github/workflows/fuzz-scheduled.yml) (30min weekly long-run with evolving corpus). **90.91% line coverage** on `roundfi-math` (110/121 lines, tarpaulin) — see [`docs/operations/test-coverage.md`](../operations/test-coverage.md).                                                                                                   |

The **53 security-specific bankrun tests** across `tests/security_*.spec.ts` + `tests/reputation_*.spec.ts` + `tests/edge_*.spec.ts` cover the first 10 rows of the matrix. The **58 app-encoder structural + 8 happy-path + 3 negative-path bankrun + 6 cargo-fuzz targets** cover the last 2 rows. The 4 Triple Shield invariants are independently captured firing on real funds on devnet — see §3.1 + `docs/devnet-deployment.md`.

**Coverage by layer:**

| Layer                             |                     Count | Lane                          | What it catches                                                  |
| --------------------------------- | ------------------------: | ----------------------------- | ---------------------------------------------------------------- |
| Bankrun security specs            |                        53 | `pnpm test:bankrun`           | Triple Shield + CPI / inputs / audit paths / lifecycle           |
| App-encoder structural            |                        58 | `pnpm test:app-encoders` (PR) | Discriminator + account count + PDA drift, < 100ms               |
| App-encoder bankrun round-trip    |                         8 | `pnpm test:bankrun`           | On-chain program accepts the encoder's bytes against valid state |
| App-encoder negative-path bankrun |                         3 | `pnpm test:bankrun`           | On-chain program correctly rejects against invalid state         |
| Math proptest                     | 6 invariants × ~250 cases | `cargo test -p roundfi-math`  | Uniform-sample invariant verification                            |
| Math cargo-fuzz (PR smoke)        |           6 targets × 60s | weekly + per-PR               | Coverage-guided mutation at branch boundaries                    |
| Math tarpaulin coverage           |    90.91% (110/121 lines) | `pnpm coverage` (advisory)    | Quantitative line coverage on the audit-critical math layer      |
