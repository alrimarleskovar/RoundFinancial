...
| #   | Threat                                         | Vector                                                                                                                         | Impact                                                                  | Mitigation status                                                                                                                                                                                   |
| --- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | **Reorg orphans a "landed" event**             | Webhook delivers tx at slot S; later S' replaces S in finalized chain; event row now references a non-existent tx              | DB has phantom row; B2B oracle includes it in score                     | 🟡 **Partial** — finality gate (32-slot wait) + orphan-marking after 256-slot grace shipped in [`reconciler.ts`](../../services/indexer/src/reconciler.ts). Prisma `orphaned: bool` column pending. |
