...
| #   | Threat                                         | Vector                                                                                                                         | Impact                                                                  | Mitigation status                                                                                                      |
| --- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| R1  | **Reorg orphans a "landed" event**             | Webhook delivers tx at slot S; later S' replaces S in finalized chain; event row now references a non-existent tx              | DB has phantom row; B2B oracle includes it in score                     | 🔵 **Pending** — finalize-only ingestion (commitment="finalized") OR finalize-aware reconciler. Today: confirmed-level |
| R2  | **Hostile RPC during backfill**                | Backfiller queries a malicious RPC; RPC returns subset of pools, lies about balances                                           | Pool state cache is wrong; reconciler doesn't notice (no second source) | 🔵 **Pending** — multi-RPC quorum for backfill (Helius + Triton + public devnet); divergence detection                 |
| R3  | **Confirmed-but-not-finalized window exploit** | User flow exploits a tx that's confirmed for 30s then dropped on reorg; B2B oracle reads "confirmed" data, returns wrong score | Downstream lending decision based on transient state                    | 🔵 **Pending** — B2B oracle reads `finalized` only; finalize-tier in event table                                       |
...
- ⛔ **Reconciler service** that joins `_unresolved` event rows ↔ canonical Pool/Member rows after configurable delay (suggested: 64 slots = ~30s). Tracked in [`#234`](https://github.com/alrimarleskovar/RoundFinancial/issues/234).
- ⛔ **Finalize-only ingestion path for B2B** — webhook can write at `confirmed`, but B2B oracle reads only rows that have a `finalizedAt IS NOT NULL` flag (set by a separate sweep)
- ⛔ **Reorg-aware DELETE** — when a tx's slot falls out of finalized chain, mark the event row `orphaned = true`; reconciler updates dependent state
- ⛔ **Multi-RPC quorum** — backfiller and reorg detector read from ≥2 independent RPC providers; divergence → alert
