...
> **Review (2026-07-03) — Caio.** Decision: **defer implementation past the pre-canary phase.** Building recovery now adds too much on-chain surface while the Genesis Canary is the priority. The permissionless `crank_payout` (SEV-051) is the accepted **interim** answer for the "member disappears → the pool must not lock" case — recovery is the separate, later concern of restoring the _member's own_ funds + reputation. Caio confirmed the critical points below (Member-PDA migration with the wallet in the seed, old→new reputation link via `IdentityRecord`, the abuse model) and added one this ADR had under-specified: **exactly what happens to a member's active positions during a pending recovery.** Next artifact: the security design doc (`docs/security/social-recovery.md`) with the full migration choreography — **no on-chain code until it lands + is reviewed.**
...
For the migration mechanics (the genuinely hard part), v1 will **close-and-recreate each `Member` PDA** under the recovery wallet — copying the financial state verbatim and transferring the position NFT — and **link the old → new wallet for reputation** by extending the existing identity-link mechanism (`IdentityRecord` / `link_passport_identity`) so scoring reads across both keys rather than trying to move immutable attestations. The exact per-account choreography, rent ownership, D/C-invariant preservation during the copy, and **what happens to a member's active positions during the pending-recovery window** — freeze them, or let contributions / claims / cranks continue against a moving target the commit must then reconcile (Caio's addition) — are all deferred to a security design doc (`docs/security/social-recovery.md`) that must land + be auditor-reviewed **before** any code.
