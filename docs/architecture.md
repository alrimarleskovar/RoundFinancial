...
| `join_pool(slot_hint?)`              | user (S)      | Pool (M), Member (M), NFT asset (M, PDA), stake_src (M), reputation_profile                       | Deposits stake, mints position NFT (asset = `[b"position-asset", pool, slot]` PDA, program-signed via invoke_signed — the wallet is the tx's ONLY signer, so mobile wallets that drop extra co-signers work), assigns slot; transitions to Active when `members_joined == members_target`                          |
...
| `claim_payout(cycle)`                | user (S)      | Pool (M), Member (M), member_dst (M), pool_usdc_vault (M), yield_vault (M)                        | Releases `credit_amount` to member at slot_index == cycle                                                                                                                                                                                                                                                          |
| `crank_payout(cycle)`                | crank         | Pool (M), Member (M), member_dst (M, member's own ATA), pool_usdc_vault (M)                       | **Liveness (SEV-051).** Permissionless payout for a live contemplated member who never claimed — after `next_cycle_at + GRACE`, delivers `credit_amount` to the member's OWN ATA and advances the cycle. Can't steal (funds only reach the member); can't run on a defaulted slot (that's `skip_defaulted_payout`) |
...
| `escape_valve_list(price)`           | member (S)    | Pool, Member, Listing (M)                                                                         | Lists position for sale (on-chain bid book)                                                                                                                                                                                                                                                                        |
...
| **Shield 2** | **Escrow Adaptativo + Stake**                       | Activates at every contemplation from cycle 2 onward | Reputation-tier-driven payout/escrow split + stake floor (Lv1 50/50/50/5m, Lv2 25/45/55/4m, Lv3 10/35/65/3m, Lv4 3/30/70/2m for stake/payout/escrow/release)        | `LEVEL_PARAMS` in stress-lab + `member.escrow_balance` / `member.stake_deposited` |
...
> **v5.2 status (ECO-V52).** Shield 2's tier table is now **4 levels**, matching the on-chain `stake_bps_for_level` schedule **50/25/10/3** (Lv2 stake 30% → 25%; new Lv4 Elite at 3%). The Lv4 escrow/payout/release parameters — previously deferred — are now specified and modelled in the Stress Lab `LEVEL_PARAMS` (**3/30/70** stake/payout/escrow, 2-month release), continuing each column's monotonic trend. The 3% stake = 33× leverage, so the Stress Lab adds an `eliteTripleDefault` preset to keep that tail visible (per ECO-001, its `netSolvency` dip is a display-only modelling signal — on-chain loss stays bounded by the D/C + Seed-Draw invariants at every tier). See §4.7.
...
- **No indefinite locks:** the pool can always advance its cycle, regardless of a single member's action or reachability. Three permissionless paths guarantee this at any contemplation slot: (a) a **defaulted** contemplated member's slot is advanced with the pot forfeited by `skip_defaulted_payout` (SEV-049); (b) a **live but absent** contemplated member (lost wallet / never claims) is paid via the permissionless `crank_payout` — it delivers `credit_amount` to the member's OWN ATA and advances the cycle after the `next_cycle_at + GRACE_PERIOD_SECS` self-claim window, so no live member can lock the pool for the others (SEV-051); (c) a late contributor is settled by `settle_default`. `claim_payout` remains the member's own fast path (no grace wait); `crank_payout` is the fallback that can't steal (funds only ever reach the member's ATA).
...
- **Cargo-fuzz:** 6 targets on `roundfi-math` (`bps`, `cascade`, `dc_invariant`, `escrow_vesting`, `seed_draw`, `waterfall`); CI advisory smoke (60s/target) + manual long-run sweep. Cumulative ~9.85B inputs (503M historical CI smoke + 600M afternoon re-validation 2026-05-24 + 8.75B overnight sweep 2026-05-24 × 6 targets × 1h each), 0 crashes across entire history, coverage stable (saturation reached) in the input space covered by the current corpus.
