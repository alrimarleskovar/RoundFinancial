...
**Cluster:** Solana **Devnet** · **Status:** ✅ All 4 programs deployed + protocol initialized + first ROSCA pool **fully cycled — slot 0 winner paid out and `pool.current_cycle` advanced from 0 to 1** (2026-05-07). `ProtocolConfig` + `ReputationConfig` + `Pool` PDAs hold real state. The four USDC vault ATAs are live: escrow holds **52.50 USDC** ($45 Lv1 stakes + $7.50 cycle-0 escrow deposits), solidarity holds **0.30 USDC** (3 × 1% of $10 installment), pool float is **~0 USDC** post-payout (drained by the $30 credit transfer to slot 0). Member 0's USDC ATA carries the **+$30 credit** they just received; their `Member.paid_out` is `true`. Four reputation attestations on-chain (3 × `SCHEMA_LATE` from cycle 0 contributions + 1 × `SCHEMA_CYCLE_COMPLETE` from the slot-0 payout). Cycles 1 and 2 follow the same pattern (contribute × 3 → claim_payout); each requires a top-up of ~$7.80 USDC into pool float (proxy for the Yield Cascade LP-distribution flow that bridges this gap in production).
...
| ---------- | ------------ | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-07 | `1a24de7`    | `64XM177Vm6zirzQnjU1juQ9TLqDsZVsCcZzfgEgVCffm` | Initial v1.1 deploy + protocol init + first pool seeded + 3 members joined + cycle 0 fully contributed + slot 0 paid out + cycle advanced 0→1 (TransferDelegate plugin + PDF-canonical waterfall + 3 PDA singletons + 1 demo Pool with 4 vault ATAs + 3 Member PDAs with Metaplex Core NFTs minted + $45 USDC escrowed + cycle-0 contributions split across solidarity / escrow / pool float + 4 reputation `Attestation` PDAs) | Seven sessions on the same day. (1) Three programs deploy. (2) `roundfi-yield-mock` deploy after 8h faucet cooldown. (3) `initialize_protocol` + `initialize_reputation` (#162). (4) `create_pool` + `init_pool_vaults` (#164) — required splitting the combined `create_pool` to dodge a Solana 3.x stack frame overflow. (5) `seed-members.ts` joining 3 Lv1 wallets (#166) — required boxing the heavyweight Account fields in `JoinPool` for the same Solana 3.x reason. (6) `seed-cycle.ts` driving 3 cycle-0 contributions (#168) — same Box pattern applied to `Contribute`, plus client-side schema selection (LATE vs PAYMENT) based on cluster `blockTime` so the Attestation PDA derives correctly. (7) `seed-claim.ts` driving the slot-0 payout (#169) — third Box application (`ClaimPayout`); also handled the structural pool-float gap with a deployer top-up of $7.80 USDC into `pool_usdc_vault` (proxy for the production Yield Cascade LP-distribution flow) before claiming, since the on-chain `WaterfallUnderflow` guard refuses `claim_payout` while `spendable < credit_amount`. The guard firing is **the protocol behaving correctly** — pool float per cycle = 74% of contributions, less than `credit_amount`. Next M3 step: drive cycles 1 + 2 (contributes + claims) and exercise `release_escrow` so members recover their stake portion at pool completion. |
