...
**Cluster:** Solana **Devnet** · **Status:** ✅ Full M3 protocol surface exercised on-chain (2026-05-07) **including the Escape Valve secondary market**. **Pool 1** (`5APoECXz…c8ooa`) ran the full 3-cycle ROSCA end-to-end (9 contribs LATE → 3 claim_payouts → `Pool.status = Completed` → `release_escrow` reverts with `EscrowLocked` durable failed-tx evidence). **Pool 2** (`8XZxRSqU…twbujm`, cycle_duration=3600s) added: ON-TIME contribs, **`deposit_idle_to_yield` + `harvest_yield` driving the full PDF-canonical waterfall** (realized 0.5 USDC, fee 0.10 → treasury, GF/LP/participants earmarks), **positive-path `release_escrow`** (member 0 received 5 USDC of vested stake), and **`escape_valve_list` + `escape_valve_buy`** — member 1 listed their position for $14, a fresh buyer wallet picked it up, atomic re-anchor closed the old Member PDA and minted a new one at the buyer's key with all bookkeeping carried over. **mpl-core owner-managed plugin bug discovered and fixed in flight**: `TransferV1` resets `FreezeDelegate` and `TransferDelegate` authorities to the new owner; the immediate re-freeze reverted with `0x1a`. Fix shipped (#176): re-approve both plugins back to `position_authority` post-transfer. Combined evidence: 4 programs deployed, 2 ROSCA pools driven across 16 distinct ix paths, 13 reputation attestations on-chain, 2 Triple Shield guards captured firing, **8 Solana 3.x Box<> workarounds applied**, 1 protocol bug surfaced + fixed end-to-end.
...
| ---------- | ------------ | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-05-07 | `ac7f781`    | `64XM177Vm6zirzQnjU1juQ9TLqDsZVsCcZzfgEgVCffm` | Full M3 protocol surface exercised on devnet across **two pools** + **Escape Valve secondary market**. Pool 1: 3-cycle ROSCA closed (9 contribs LATE → 3 claim_payouts → Completed → release_escrow reverts with EscrowLocked). Pool 2 (cycle_duration=3600s): ON-TIME contribs + yield Cascade (realized 0.5 USDC, fee 0.10 → treasury) + positive release_escrow (member 0 +5 USDC) + **escape_valve_list + escape_valve_buy** (member 1's slot 1 sold to a fresh buyer for $14, atomic re-anchor of Member PDA + NFT). **Found-and-fixed protocol bug**: mpl-core's `TransferV1` resets owner-managed plugin authorities (FreezeDelegate / TransferDelegate) — fix re-approves both back to position_authority post-transfer. 13 reputation `Attestation` PDAs, 2 Triple Shield guards captured firing, 8 Solana 3.x Box workarounds applied, 1 protocol bug surfaced + fixed end-to-end. | Eleven sessions on the same day, culminating in #176. (1)–(10) per the previous cumulative entry: protocol deploy → init → Pool 1 lifecycle (3 cycles + claims + EscrowLocked negative) → Pool 2 + Yield Cascade + positive release_escrow (#160-#174). (11) **Escape Valve list + buy (#176)**: refactor extends to support seller/buyer flows; Box on `EscapeValveList` + `EscapeValveBuy`; new drivers `seed-evlist` + `seed-evbuy`. The first buy attempt against pool 2 / slot 1 reverted with mpl-core 0x1a — `TransferV1` resets owner-managed plugin authorities so the immediate re-freeze loses its authority. The bankrun harness only exercised defense-in-depth `AssetNotRefrozen` paths (atomic-revert-only, never the positive flow). Fix shipped same session: insert `ApprovePluginAuthorityV1` × 2 between transfer and re-freeze, signed by buyer_wallet (the new owner), re-delegating both FreezeDelegate AND TransferDelegate back to position_authority. Post-fix, the buy went through atomically: $14 buyer→seller + close old Member PDA + init new Member PDA at buyer's key (snapshot carryover) + thaw/transfer/re-delegate/re-freeze NFT + close listing. The pre-fix simulation revert (no on-chain signature) is a development artifact; the fix landed and the post-fix tx is durable evidence the corrected flow is correct. Next M3 step: app↔chain wiring (front-end modals firing real txs against the deployed pools). |
