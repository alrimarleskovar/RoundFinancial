...
**Cluster:** Solana **Devnet** · **Status:** ✅ All 4 programs deployed + protocol initialized + first ROSCA pool fully seeded with 3 members (2026-05-07). `ProtocolConfig` + `ReputationConfig` + `Pool` PDAs hold real state. The four USDC vault ATAs are live and the escrow vault holds **45 USDC** (3 × $15 Lv1 stake) from the three members, each with their own Metaplex Core position NFT minted in the same `join_pool` tx. Live cycle progression (contribute / claim_payout / harvest_yield / settle_default) is the next M3 step — gated behind app↔on-chain wiring.
