...
**Cluster:** Solana **Devnet** · **Status:** ✅ All 4 programs deployed + protocol initialized + first ROSCA pool **fully completed end-to-end — all 3 cycles closed, all 3 members paid out, `Pool.status = Completed`** (2026-05-07). `ProtocolConfig` + `ReputationConfig` + `Pool` PDAs hold real state. The four USDC vault ATAs carry the closing balances: **escrow holds ~$67.50 USDC** ($45 Lv1 stakes + 3 × 3 × $2.50 escrow deposits across the 3 cycles), **solidarity holds ~$0.90 USDC** (3 × 3 × $0.10 from the 1% solidarity split), pool float drained to ~0 by the three $30 credit transfers. All 3 members carry `paid_out=true`; each member's USDC ATA holds the **+$30 credit** they received in their respective slot. **12 reputation attestations on-chain**: 9 × `SCHEMA_LATE` (3 contributions per member × 3 cycles, all marked LATE since the 60-second `cycle_duration` had elapsed) + 3 × `SCHEMA_CYCLE_COMPLETE` (one per slot's payout). **`release_escrow` Triple Shield enforcement verified on-chain**: a negative-path call against member 0 reverted with `EscrowLocked` (error code `6011 / 0x177B`) because `member.on_time_count (0) < args.checkpoint (1)` — the protocol refused to release the legitimate $22.50 escrow balance because all contributions had been LATE. **No Triple Shield bypass.**
...
| ---------- | ------------ | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-07 | `c9e8aa5`    | `64XM177Vm6zirzQnjU1juQ9TLqDsZVsCcZzfgEgVCffm` | Initial v1.1 deploy + protocol init + first pool **fully completed end-to-end** — `create_pool` → 3 × `join_pool` (Lv1 stakes, Metaplex Core position NFTs) → 3 × `contribute` × 3 cycles (9 total) → 3 × `claim_payout` (slots 0 → 1 → 2) → `Pool.status = Completed` (12 reputation `Attestation` PDAs total: 9 × `SCHEMA_LATE` + 3 × `SCHEMA_CYCLE_COMPLETE`). **Plus `release_escrow` Triple Shield enforcement verified**: a negative-test against member 0 reverted on-chain with `EscrowLocked` (code 6011 / 0x177B) because contributions had been LATE — the protocol refused to release the legitimate $22.50 escrow balance, demonstrating the on-time discipline guard fires on real funds. | Nine sessions on the same day. (1) Three programs deploy. (2) `roundfi-yield-mock` deploy after 8h faucet cooldown. (3) `initialize_protocol` + `initialize_reputation` (#162). (4) `create_pool` + `init_pool_vaults` (#164) — required splitting the combined `create_pool` to dodge a Solana 3.x stack frame overflow. (5) `seed-members.ts` joining 3 Lv1 wallets (#166) — required boxing the heavyweight Account fields in `JoinPool` for the same Solana 3.x reason. (6) `seed-cycle.ts` driving cycle-0 contributions (#168) — same Box pattern applied to `Contribute`, plus client-side schema selection (LATE vs PAYMENT) based on cluster `blockTime`. (7) `seed-claim.ts` driving slot-0 payout (#169) — third Box application (`ClaimPayout`); handled the structural pool-float gap with a deployer top-up of $7.80 USDC into `pool_usdc_vault` (proxy for the production Yield Cascade LP-distribution flow). (8) Re-ran `seed-cycle` + `seed-claim` for cycles 1 + 2 (#171) — same scripts, no code changes needed. **Pool reached `Completed` status.** (9) `seed-release.ts` negative test (#172) — fifth Box application (`ReleaseEscrow`); used `skipPreflight=true` so the failed tx LANDS on-chain with a durable Solscan link. The on-chain log captured `AnchorError ... release_escrow.rs:91. Error Code: EscrowLocked. Error Number: 6011`. Two Triple Shield guards now have on-chain failed-tx evidence: `WaterfallUnderflow` (top-up flow) and `EscrowLocked` (release flow). Next M3 step: drive a positive-path release in a fresh pool with longer `cycle_duration`, or pivot to `deposit_idle_to_yield` + `harvest_yield` to replace the manual top-up proxy with the real Yield Cascade. |
