...
**Update 2026-05-20 — stretch goal CLOSED at 3/3:** operator topped up CI deployer wallet from 19.76 → 23.76 SOL via `solana transfer` from a local devnet wallet (CLI airdrop + laptop-side faucets had rate-limited; documented as a top-up strategy in the rehearsal-3 log); rehearsal **3** ran green end-to-end consuming **12.62 SOL — identical to 2b to 4 sig figs**. Same workflow, fresh keypairs (different from both 1g and 2b IDs as expected), all 4 programs deployed cleanly. The `cd-pipeline.md` §"Rehearsal protocol" Step 6 ("3 consecutive clean runs") is now satisfied. See [`2026-05-20-SEV-046-rehearsal-3-success.md`](./2026-05-20-SEV-046-rehearsal-3-success.md) for the full run record + the 1g-vs-2b-vs-3 three-way reproducibility comparison + operator top-up strategy notes. **Stretch goal closure is reproducibility-confidence-only — does NOT unblock mainnet GA** (still gated on §3.1 audit + legal + multi-sig + #230 SDK-transitive bump + #233 Kamino part B).
...
| 3-aborted | `devnet-deploy-v20260520-rehearsal-3`  | **Balance gate fired pre-deploy (second independent confirmation of feature working as designed).** Deployer pubkey `5ZpFt...` had **19.76 SOL** post-2b — 0.24 SOL below the 20 SOL floor. Step "Check deployer balance ≥ 20 SOL" exit 1, same shape as 2-aborted on 2026-05-19. Operator's first unblock attempts (`solana airdrop` + laptop-side web faucet) hit rate limits | — (no fix needed)                                                  | Same pattern as 2-aborted — the floor refusing to start the workflow when SOL < deploy cost + safety margin. Operator unblock paths confirmed: web faucet pool independent of CLI pool, `solana transfer` from local wallet to CI works, alt faucets viable. **Not a code bug — operator-side top-up cycle**                                                                                                                                                                                                                                                                                                                            |
| 3         | (re-run of `devnet-deploy-v20260520-rehearsal-3` tag via UI, post-transfer-topup) | ✓ **Green.** Third successful end-to-end deploy. Operator topped CI deployer wallet 19.76 → 23.76 SOL via `solana transfer` from a local wallet (CLI + laptop faucet rate-limited; transfer-from-local was the unblock path that worked); 12.62 SOL consumed — **identical to 2b to 4 sig figs**. 4 programs deployed cleanly with fresh keypairs. See [`2026-05-20-SEV-046-rehearsal-3-success.md`](./2026-05-20-SEV-046-rehearsal-3-success.md) for the run record + 1g-vs-2b-vs-3 three-way reproducibility comparison + operator top-up strategy notes. **3× stretch goal CLOSED at 3/3.** | —                                                                  | Workflow ran exactly as spec'd. No new failure modes across 3 consecutive runs. `cd-pipeline.md` §"Rehearsal protocol" Step 6 (3-consecutive-clean) satisfied. **Closure is reproducibility-confidence-only — does NOT unblock mainnet GA** (still §3.1 + §3.2 hard-gates standing)                                                                                                                                                                                                                                                                                                                                                                                                                |
