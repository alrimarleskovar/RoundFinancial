...
- [ ] **Balance ≥ 20 SOL** on the deployer wallet. Per the SEV-046 rehearsal saga (`docs/operations/rehearsal-logs/2026-05-18-SEV-046-rehearsal-saga.md`), a full 4-program deploy from a clean slate costs ~12.5 SOL (rent-exempt = 2× bytecode: core ~6.14, reputation ~2.6, yield_kamino ~2.0, yield_mock ~1.8); the 20 SOL floor is a 1.6× buffer for priority-fee spikes. Incremental upgrades cost ~0.5–2 SOL per program (existing rent-exempt buffer is reused). Devnet: `solana airdrop 5 --url devnet` (rate-limited 2× per 8h; fall back to https://faucet.solana.com). Mainnet: real SOL via treasury top-up before tagging.
- [ ] **`solana` CLI on PATH**, version matches the deployment target. Today: pinned to Solana Agave **3.0.0** in both CI (`anchor · build` lane) and CD pipeline (`devnet-deploy.yml` + `mainnet-deploy.yml`). See [`docs/verified-build.md`](../verified-build.md). The mainnet ceremony will use the same Agave 3.0.0 pin. (Issue #230 also tracks the SDK transitive bump — `anchor-lang 0.30.1` still pulls `solana-program 1.18.x` — which is independent of the CLI version and stays open until anchor 0.31+ lands.)
