...
ANCHOR_TOML_EXPECTED=$(grep -E '^[a-z_]+ +=' Anchor.toml | head -10 || true)
ID_MISMATCH=0
for prog in "${PROGRAMS[@]}"; do
  if [[ -f "target/deploy/${prog}-keypair.json" ]]; then
    ...
    # Compare against Anchor.toml [programs.localnet]. The `=~` is a substring
    # check — Anchor.toml has the pubkey on one of the localnet lines.
    if echo "$ANCHOR_TOML_EXPECTED" | grep -q "$pubkey"; then
      echo "    $prog: $pubkey ✓"
    else
      echo "    $prog: $pubkey ✗ (does NOT match Anchor.toml — see warning below)"
      ID_MISMATCH=1
    fi
  fi
done

if [[ "$ID_MISMATCH" == 1 ]]; then
  echo ""
  echo "  ⚠  Program IDs drift from Anchor.toml — specs WILL fail with 'program not found'."
  echo ""
  echo "  Cause: target/deploy/*-keypair.json were freshly generated by 'anchor build'"
  echo "  (first run on this machine), so their pubkeys don't match the canonical IDs"
  echo "  in Anchor.toml + declare_id! macros."
  echo ""
  echo "  Fix — run once per machine:"
  echo "    anchor keys sync                         # auto-rewrites Anchor.toml + declare_id!"
  echo "    bash scripts/test-fresh.sh               # re-run to redeploy at the new IDs"
  echo ""
  echo "  Caveat: 'anchor keys sync' modifies tracked source files (Anchor.toml + 4 lib.rs"
  echo "  with declare_id!). Don't commit those changes — they're machine-local. The canonical"
  echo "  IDs on main are protocol-level facts and must NOT drift."
fi
