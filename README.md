...
| 9. Security audit          | ⏳                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 10. Devnet testing         | 🟢 **Full ROSCA closed end-to-end on devnet** — `create_pool` → 3 × `join_pool` (Lv1 stakes + position NFTs) → 9 × `contribute` (3 cycles × 3 members, split across solidarity / escrow / float) → 3 × `claim_payout` (slots 0 → 1 → 2) → **`Pool.status = Completed`** → 12 reputation `Attestation` PDAs (9 × `SCHEMA_LATE` + 3 × `SCHEMA_CYCLE_COMPLETE`). **Plus negative-test evidence**: `release_escrow` reverts on-chain with `EscrowLocked` (code 6011) when `on_time_count < checkpoint` — Triple Shield enforcement of on-time discipline confirmed on real funds. See [`docs/devnet-deployment.md`](docs/devnet-deployment.md). |
