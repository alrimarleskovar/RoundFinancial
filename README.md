...
<p>
  ...
  <a href="docs/devnet-deployment.md"><img alt="Devnet · M3 + Escape Valve" src="https://img.shields.io/badge/Devnet-M3_%2B_Escape_Valve-00C8FF?style=for-the-badge&logo=solana&logoColor=06090F"/></a>
</p>
...
| 9. Security audit          | ⏳                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 10. Devnet testing         | 🟢 **Full M3 protocol surface exercised on devnet** across two pools + Escape Valve secondary market. Pool 1: 3-cycle ROSCA closed (9 contribs LATE → 3 claim_payouts → Completed) + EscrowLocked negative test. Pool 2 (cycle_duration=3600s): ON-TIME contribs + Yield Cascade (realized 0.5 USDC, fee 0.10 to treasury) + positive release_escrow (member 0 +5 USDC) + **`escape_valve_list` + `escape_valve_buy`** (member 1's slot sold to a fresh buyer for $14, atomic re-anchor of Member PDA + NFT). **Real mpl-core bug surfaced and fixed end-to-end**: `TransferV1` resets owner-managed plugin authorities; fix re-approves them post-transfer. 8 Solana 3.x Box workarounds + 1 protocol fix shipped. See [`docs/devnet-deployment.md`](docs/devnet-deployment.md). |
...
> **M3 protocol surface fully exercised on-chain, plus a real protocol bug found and fixed.** Across pool 1 (negative paths + claims) and pool 2 (yield + positive release + Escape Valve secondary market), every active M3 instruction now has a real on-chain receipt: `create_pool`, `init_pool_vaults`, `join_pool`, `contribute`, `claim_payout`, `release_escrow`, `deposit_idle_to_yield`, `harvest_yield`, `escape_valve_list`, `escape_valve_buy`, plus `roundfi_yield_mock.init_vault`. The two Triple Shield guards (`WaterfallUnderflow`, `EscrowLocked`) are captured firing on real funds. **The mpl-core owner-managed plugin behavior** (a real production-relevant gotcha bankrun missed) was surfaced and fixed end-to-end on devnet. 8 Solana 3.x Box workarounds + 1 protocol fix shipped this milestone.
