...
> **Internal pre-audit — May 2026.** A 4-pass red-team exercise run by the RoundFi team **simulating the Adevar Labs methodology** (formal external engagement scoping in progress), explicitly designed to surface and close findings before the paid auditor's clock starts. 34 findings catalogued; 31 closed in code (Critical/High 9/9), 1 upstream-blocked, 2 acknowledged-by-design.
>
...
| 11. Mainnet migration      | 🟡 **Canary code controls + internal pre-audit complete; formal Adevar engagement + Squads ceremony + bug bounty pending.** Code-side (this sprint): Kamino harvest CPI ([#309](https://github.com/alrimarleskovar/RoundFinancial/pull/309)), TVL caps per-pool + protocol-wide ([#310](https://github.com/alrimarleskovar/RoundFinancial/pull/310) + [#314](https://github.com/alrimarleskovar/RoundFinancial/pull/314) orphan-PDA fix), yield-adapter allowlist + lock-flag ([#311](https://github.com/alrimarleskovar/RoundFinancial/pull/311) + [#316](https://github.com/alrimarleskovar/RoundFinancial/pull/316)), devnet/mainnet network-confusion banner ([#312](https://github.com/alrimarleskovar/RoundFinancial/pull/312)), Squads multisig procedure + PDA-derivation utility ([#313](https://github.com/alrimarleskovar/RoundFinancial/pull/313)), negative bankrun coverage on canary controls ([#315](https://github.com/alrimarleskovar/RoundFinancial/pull/315)), Civic→Human-Passport PoP provider migration ([#317](https://github.com/alrimarleskovar/RoundFinancial/pull/317)). **Internal pre-audit** (May 2026 — 4-pass red-team simulating Adevar Labs methodology, run by the team _before_ commissioning the formal engagement so the paid auditor's clock isn't burned on findings we can surface ourselves; 34 findings catalogued in [`docs/security/internal-audit-findings.md`](docs/security/internal-audit-findings.md)): 2 Critical, 7 High, 9 Medium, 10 Low, 6 Informational. **31 🟢 closed** (Critical/High 9/9) in PRs [#326..#351](https://github.com/alrimarleskovar/RoundFinancial/pulls?q=is%3Apr+is%3Amerged+SEV) including timelocked authority rotation (SEV-021/022 High), c-token redirect fix (SEV-001 Critical), grace-period revert (SEV-002 Critical), pool viability runtime guard (SEV-031), `release_escrow` partial-pay overpay regression fix with 6 negative tests (SEV-029 High), fee_bps_yield 1-day timelock (SEV-024 follow-up). **1 🟠 upstream-blocked** (SEV-012 bankrun-in-CI ← mpl-core Anchor 0.31 support). **2 🔵 design-intentional** (SEV-018 settle_default pause bypass, SEV-032 ReputationConfig padding exhausted). External pending: **Adevar Labs formal engagement** (scoping in progress), **3-of-5 Squads multisig ceremony** (hardware-wallet quorum), **bug bounty live** (at GA), **legal counsel review** (Q3 2026). See [`MAINNET_READINESS.md`](MAINNET_READINESS.md) for the full checklist. |
