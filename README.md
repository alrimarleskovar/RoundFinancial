...
<sub>**Status:** M3 shipped · 4 programs live on devnet · reproducible-build attestation on-chain · **mainnet-canary code controls complete** (Kamino harvest CPI, TVL caps per-pool + protocol-wide, yield-adapter allowlist + lock-flag, devnet→mainnet network-confusion banner, Squads multisig procedure docs, Civic→Human-Passport PoP migration) · **Internal pre-audit complete (3-pass red-team simulating Adevar Labs methodology, pre-engagement preparation)** — 33 findings catalogued, **30 🟢 closed**, 1 🟠 upstream-blocked (mpl-core borsh), 2 🔵 design-intentional ([tracker](docs/security/internal-audit-findings.md)) · **Formal external audit (Adevar Labs) — engagement scoping in progress** · **270+ tests across 22 spec files** (53 security-specific bankrun + 58 app-encoder structural + 7 bankrun round-trips + 10 canary-control negative + 36 audit-regression unit/proptest + 109 lifecycle/edge/parity) · 6 cargo-fuzz targets on `roundfi-math` · **Adevar engagement + Squads ceremony + bug bounty pending** before mainnet GA — see [`MAINNET_READINESS.md`](MAINNET_READINESS.md). _Last updated: May 2026._</sub>
...
> **Internal pre-audit — May 2026.** A 3-pass red-team exercise run by the RoundFi team **simulating the Adevar Labs methodology** (formal external engagement scoping in progress), explicitly designed to surface and close findings before the paid auditor's clock starts. 33 findings catalogued; 30 closed in code, 1 upstream-blocked, 2 acknowledged-by-design.
>
...
| 11. Mainnet migration      | 🟡 **Canary code controls + internal pre-audit complete; formal Adevar engagement + Squads ceremony + bug bounty pending.** Code-side (this sprint): Kamino harvest CPI ([#309](https://github.com/alrimarleskovar/RoundFinancial/pull/309)), TVL caps per-pool + protocol-wide ([#310](https://github.com/alrimarleskovar/RoundFinancial/pull/310) + [#314](https://github.com/alrimarleskovar/RoundFinancial/pull/314) orphan-PDA fix), yield-adapter allowlist + lock-flag ([#311](https://github.com/alrimarleskovar/RoundFinancial/pull/311) + [#316](https://github.com/alrimarleskovar/RoundFinancial/pull/316)), devnet/mainnet network-confusion banner ([#312](https://github.com/alrimarleskovar/RoundFinancial/pull/312)), Squads multisig procedure + PDA-derivation utility ([#313](https://github.com/alrimarleskovar/RoundFinancial/pull/313)), negative bankrun coverage on canary controls ([#315](https://github.com/alrimarleskovar/RoundFinancial/pull/315)), Civic→Human-Passport PoP provider migration ([#317](https://github.com/alrimarleskovar/RoundFinancial/pull/317)). **Internal pre-audit** (May 2026 — 3-pass red-team simulating Adevar Labs methodology, run by the team _before_ commissioning the formal engagement so the paid auditor's clock isn't burned on findings we can surface ourselves; 33 findings catalogued in [`docs/security/internal-audit-findings.md`](docs/security/internal-audit-findings.md)): 2 Critical, 6 High, 9 Medium, 10 Low, 6 Informational. **30 🟢 closed** in PRs [#326..#347](https://github.com/alrimarleskovar/RoundFinancial/pulls?q=is%3Apr+is%3Amerged+SEV) including timelocked authority rotation (SEV-021/022 High), c-token redirect fix (SEV-001 Critical), grace-period revert (SEV-002 Critical), pool viability runtime guard (SEV-031), `release_escrow` partial-pay overpay regression fix with 6 negative tests (SEV-029 High), fee_bps_yield 1-day timelock (SEV-024 follow-up). **1 🟠 upstream-blocked** (SEV-012 bankrun-in-CI ← mpl-core Anchor 0.31 support). **2 🔵 design-intentional** (SEV-018 settle_default pause bypass, SEV-032 ReputationConfig padding exhausted). External pending: **Adevar Labs formal engagement** (scoping in progress), **3-of-5 Squads multisig ceremony** (hardware-wallet quorum), **bug bounty live** (at GA), **legal counsel review** (Q3 2026). See [`MAINNET_READINESS.md`](MAINNET_READINESS.md) for the full checklist. |
