...
<p>
  ...
  <a href="docs/devnet-deployment.md"><img alt="Devnet · pool completed" src="https://img.shields.io/badge/Devnet-pool_completed-00C8FF?style=for-the-badge&logo=solana&logoColor=06090F"/></a>
</p>
...
| 9. Security audit          | ⏳                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 10. Devnet testing         | 🟢 **Full ROSCA closed end-to-end on devnet** — `create_pool` → 3 × `join_pool` (Lv1 stakes + position NFTs) → 9 × `contribute` (3 cycles × 3 members, split across solidarity / escrow / float) → 3 × `claim_payout` (slots 0 → 1 → 2) → **`Pool.status = Completed`** → 12 reputation `Attestation` PDAs (9 × `SCHEMA_LATE` + 3 × `SCHEMA_CYCLE_COMPLETE`). See [`docs/devnet-deployment.md`](docs/devnet-deployment.md). `release_escrow` (members recover stake) next. |
...
> **Full ROSCA closed end-to-end on devnet.** Protocol init → pool create → vault inits → 3 members joined with USDC stakes + position NFTs → 9 contributions across 3 cycles → 3 payouts (slots 0 → 1 → 2) → **`Pool.status = Completed`** → 12 reputation attestations on-chain (9 × `SCHEMA_LATE` + 3 × `SCHEMA_CYCLE_COMPLETE`). Next: `release_escrow` so members recover their stake portion. See [`docs/status.md`](docs/status.md) and [`docs/devnet-deployment.md`](docs/devnet-deployment.md).
